...
from Helper.QRCode.myqrcode import MyQRCode
...
# Custom filter function for base64 encoding
...
def show_vcard():
    ...
    for vacrd in vcards:
        if len(vacrd.qrcode):
            print(vacrd.qrcode[0].qr_code_img)
...
def insertdata(card_id):
    ...
    qrcode_data = QRCode(
        ...
        qr_code_img= bytes(MyQRCode(data="http://127.0.0.1:4444" + str(card_id)).qr_base64, 'utf-8'),
        vcard_id=card_id
    )
